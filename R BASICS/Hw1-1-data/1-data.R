...
## [your code here]
sel <-  select(df,X,Grad.Rate)

sel
sel2 <- filter(sel,Grad.Rate>=80)
sel2

sel3 <- arrange(sel2,desc(Grad.Rate))
sel3


##Another method

arrange(filter(select(df,X,Grad.Rate),Grad.Rate>=80),desc(Grad.Rate))
...
df1 <- group_by(df,Private)
df1<- summarise(df1, max = max(Outstate), min(Outstate))
df1

college1 <- group_by(college, Private)
college1 <- summarise(college1, max = max(Outstate), min(Outstate))
college1
